...
const Store = () => {
    ...
    const getIcon = (type) => {
        switch (type) {
            ...
            case 'palette': return <Star className="w-12 h-12 text-rose-500 drop-shadow-[0_0_10px_rgba(244,63,94,0.3)]" />;
            case 'sticker': return <Star className="w-12 h-12 text-indigo-500 drop-shadow-[0_0_10px_rgba(99,102,241,0.3)]" />;
            case 'box': return <Star className="w-12 h-12 text-teal-500 drop-shadow-[0_0_10px_rgba(20,184,166,0.3)]" />;
            case 'book': return <Star className="w-12 h-12 text-amber-600 drop-shadow-[0_0_10px_rgba(217,119,6,0.3)]" />;
            ...
            <AnimatePresence>
                {notification && (
                    <motion.div 
                        ...
                        className={`fixed bottom-10 right-10 z-50 px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-4 font-bold border-2 ${
                            ...
                            >
                                ...
                                <div className="absolute top-6 right-8">
                                    <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest ${
                                        item.category === 'Symbolic' ? 'bg-emerald-50 text-emerald-600' :
                                        item.category === 'Premium' ? 'bg-orange-50 text-orange-600' :
                                        item.category === 'Student' ? 'bg-purple-50 text-purple-600' :
                                ...
                                <div className="bg-[#f8f9fa] rounded-[2rem] p-4 mb-8 flex items-center justify-center transition-all duration-500 group-hover:bg-emerald-50 relative overflow-hidden h-64">
                                    ...
                                    {/* Background Glow */}
                                    <div className={`absolute w-32 h-32 blur-[60px] opacity-20 group-hover:opacity-40 transition-opacity duration-500 ${
                                        item.category === 'Symbolic' ? 'bg-emerald-400' :
                                        item.category === 'Premium' ? 'bg-orange-400' :
                                        item.category === 'Student' ? 'bg-purple-400' :
                                        'bg-blue-400'
                                    }`}></div>
                                ...
                                <p className="text-gray-400 font-medium mb-10 leading-relaxed h-14 line-clamp-2 text-sm">
                                    {item.description}
                                </p>
                                ...
                                <div className="mt-auto flex items-center justify-between gap-5 pt-8 border-t border-gray-50">
                                    ...
                                    <button 
                                        ...
                                        className={`flex-1 py-4 rounded-2xl font-extrabold text-sm transition-all duration-300 shadow-lg active:scale-95 uppercase tracking-wider ${
                                            purchasing === item.id 
